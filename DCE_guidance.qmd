...
## Some other Published Guidance for Engineers
 ...
 - Similarly, a guidance document for the nuclear industry [@Martin2019] has similar limitations. This document makes reference to Bayesian methods and structural reliability, but does not provide any guidance on how to interpret the analysis. For instance, the use of "*conservative defined distributions*" to calculate a "*conservative failure probability*" is discussed. Conservative modelling assumptions have traditionally be applied instead of quantifying uncertainty, and attempts to combine these two approaches (perhaps as some sort of compromise or stepping stone) can lead to confusion, since they have different use cases. This explanation is detaukled further in [@DiFrancesco2022].
 - The most comprehensive guidance on modelling uncertainty in the built environment can be found in the probabilistic model code, developed by the joint committee of structural safety [@JCSS2001]. This series of documents considers uncertainties in applied loads, material properties, measurements, and model predictions. Much of this guidance was incorporated in the COST Action TU1402 project case studies on quantifying the expected value of structural health monitoring systems [@Straub2017], which has lead to many demonstrations of this approach in scientific literature in recent years.
  - There are also several engineering textbooks that introduce concepts in uncertainty quantification, and decision support in the context of engineering challenges [@Benjamin2014c], [@Jordaan2005], and [@Faber2012]. These books all consider, to some extent, quantification of the expected value of data to engineers.
 
This document is intended to provide an update to the aforementioned industrial guidance and more accessible examples than those included by other publications by making all data and all code available, and inviting contributions from readers. It also addresses some of the challenges that were associated with computational statistics in previous decades, such as characterisation of dependencies in multi-variate probabilistic models, and interpretation of results of structural reliability analysis.
...
Latin hypercube sampling ensures more evenly spaced samples than standard (inverse-transform, Monte Carlo sampling) [@Olsson2003]. The effect of this is shown in @fig-it_lhs.
...
One method for achieving this is to use copula functions [@Joe2015]. There are various types of copula, but in each case they describe the dependency between the different constituent (marginal) components of a multi-variate probabilistic model.
...
In this example, some *prior* models (starting points) for the mean and standard deviation of the yield strength are shown below. These, like all other details about the structure of this model, are a subjective feature, but sampling from these priors can be a helpful tool in agreeing suitable values [@Gelman2020a]. An example of this is provided later in this document.
...
The problem is described by the influence diagram in @fig-prior_id_heat_pumps. Here, heat pump parameters, $\alpha$ and $\beta$ describe the degradation and improvements to the performance of a heat pump, respectively. Heat pump efficiency, characterised by a Seasonal Performance Factor (SPF), is calculated based on how the system degrades with age (using $\alpha$), and how maintenance activities can improve, or restore, the performance (using $\beta$).
...
$$
$$
\alpha \sim Beta(\alpha = 2, \beta = 20)
$$
Where: $\beta_{A} = 0.15$, $\beta_{B} = 1.5$, and $\gamma = 2.0$. The cost of a maintenance activity is equal to $\$2,000$, and is assumed to scale linearly, i.e. $N$ maintenance activities would cost $N \times \$2, 000$. The cost of electricity for the system, $C_{e}$ is calculated, using the heating load in kWh, $L_{H}$, the price per kWh, in GBP $p_{e}$, and the SPF:
...
```{R}
get_beta <- function(n_maintenance, a = 0.15, b = 1.5, gamma = 2.0){
  (a * n_maintenance^gamma) / (b + n_maintenance^gamma)
}

n_samples <- 1e3
heat_load_kWh <- 1e6; cost_per_kWh <- 0.50; cost_maint <- 2e3

max_n_maint <- 12; n_maint <- seq(from = 0, to = max_n_maint, by = 1)

alpha_prior_samples <- (lhs(n_samples, 1) |> qbeta(shape1 = 2, shape2 = 20))[,1]
...
```{R}
prior_decision <- function(n_maint_range = n_maint, alpha = list(alpha_prior_samples)){
  
  tibble(n_maint_range, alpha) |>
  mutate(beta = get_beta(n_maintenance = n_maint_range),
         SPF = SPF_initial,
         maint_cost = n_maint_range * cost_maint,
         ...
         SPF = map(.x = SPF, .f = function(.x){
           lapply(X = .x, FUN = function(x){ifelse(x <= SPF_initial, x, SPF_initial)}) |> unlist()}),
         elec_cost = map(.x = SPF,
                         .f = function(.x){cost_per_kWh * heat_load_kWh / .x}),
         total_cost = map2(.x = elec_cost, .y = maint_cost,
                           .f = function(.x, .y){ .x + .y})) |> 
    group_by(n_maint_range) |>
...
prior_df <- prior_decision()

ggplot(data = prior_df, mapping = aes(x = n_maint_range, y = exp_cost))+
  geom_line(mapping = aes(lty = "Expected cost"))+
  scale_x_continuous(name = 'Number of scheduled maintenance activities per year', 
                     breaks = seq(from = 0, to = 12, by = 1))+
  scale_y_continuous(name = 'Total (annual) operational costs', labels = scales::dollar)+
  geom_vline(mapping = aes(lty = 'Expected optimal number \nof maintenance activities',
                           xintercept = prior_df[which.min(prior_df$exp_cost),]$n_maint_range))+
...
The expected optimal number of maintenance activities to schedule (that which is associated with the lowest expected cost) is `r prior_df[which.min(prior_df$exp_cost),]$n_maint_range`, with an associated expected cost of £`r prior_df[which.min(prior_df$exp_cost),]$exp_cost |> signif(digits = 4)`.
...
```{R}
prepost_decision <- function(n_maint_range = n_maint, alpha){
  
  prepost_df <- tibble()
  for (a in alpha){
    prepost_df <- bind_rows(prepost_df,
                            (prior_decision(alpha = a) |> dplyr::arrange(exp_cost))[1, ] |>
                              dplyr::select(c(n_maint_range, exp_cost)) |> mutate(alpha = a))
  }
  return(prepost_df)
}
```
...
prior_cost <- ((prior_df |> dplyr::arrange(exp_cost))[1, ] |> dplyr::select(c(n_maint_range, exp_cost)))$exp_cost
prepost_df <- prepost_decision(alpha = alpha_prior_samples)

VoPI <- prior_cost - mean(prepost_df$exp_cost) 
...
beta_dist_summary <- function(shape_1, shape_2, C = 1){
  
  var <- C^2 * shape_1 * shape_2 / ((shape_1 + shape_2)^2 * (shape_1 + shape_2 + 1))
  mean <- C * shape_1 / (shape_1 + shape_2)
...
VoPI_SA_df <- tibble()
for (shape_1 in c(1/2, 1, 2, 5, 10, 20, 50)){

  shape_2 <- 10 * shape_1; set.seed(seed = 240819)
  alpha_prior_samples <- (lhs(n_samples, 1) |> qbeta(shape1 = shape_1, shape2 = shape_2))[,1]
  
  var_alpha <- beta_dist_summary(shape_1 = shape_1, shape_2 = shape_2)["variance"] |> unlist()
  
  prior_cost <- ((prior_decision(alpha = list(alpha_prior_samples)) |> 
                    dplyr::arrange(exp_cost))[1, ] |> 
                   dplyr::select(c(n_maint_range, exp_cost)))$exp_cost
  
  prepost_cost <- prepost_decision(alpha = alpha_prior_samples)$exp_cost |> mean()

  VoPI_SA_df <- bind_rows(VoPI_SA_df, 
                          tibble(alpha = shape_1,
                                 alpha_var = var_alpha,
...
ggplot(data = VoPI_SA_df, mapping = aes(x = alpha_var, y = VoPI))+
  ...
  scale_x_continuous(name = expression(paste('Uncertainty (variance) in prior model of ', alpha)))+
  scale_y_continuous(name = 'Expected value of (perfect) smart meter data', labels = scales::dollar, 
                     limits = c(0, VoPI_SA_df$VoPI |> max() + 3e2))+
  ggthemes::theme_base(base_size = 12, base_family = "Atkinson Hyperlegible")+
  theme(plot.background = element_rect(color = NA))
```
...
An environment for growing various produce, such as the underground farm project [@Jans-Singh2020], requires monitoring of features that will impact crop survival. Being able to forecast availability of crops is important when agreeing future sales.
...
prior_df = solve_id(β = mean(reliabilities))
...
```{r}
#| echo: false
#| fig-cap: Simulations of Possible Outcomes From a Digital Twin Verification Study, and Associated Expected Optimal Actions
#| label: fig-voi_results

arrow_df <- tibble(xend = prior_df$u_opt,
                   x = verif_df$u_opt |> mean(),
                   y = 2.25, yend = 2.25)

ggplot(data = verif_df |> mutate(a_opt = factor(x = a_opt, levels = c("no_action", "investigate"))), 
       mapping = aes(x = u_opt, y = a_opt))+
  geom_jitter(shape = 21, alpha = 1/2, height = 1/3, mapping = aes(fill = β))+
  geom_vline(mapping = aes(xintercept = prior_df$u_opt, 
                           lty = "Expected Cost Without Verification"), alpha = 1/2) +
  geom_vline(mapping = aes(xintercept = verif_df$u_opt |> mean(), 
                           lty = "Expected Cost With Verification"), alpha = 1/2)+
  ...
  geom_text(data = arrow_df, 
            mapping = aes(x = c(x, xend) |> mean(), y = 2.5, label = paste("$", signif(x = (xend - x), digits = 7))),
            size = 3.5, family = "Atkinson Hyperlegible")+
  scale_color_manual(values = c("midnightblue"))+
  scale_fill_viridis_c()+
  labs(y = "Expected Optimal Action", fill = "DT Reliability", lty = "", col = "")+
    scale_linetype_manual(values = c(2, 1))+
  scale_x_continuous(name = "Expected Cost", labels = scales::dollar_format(), breaks = scales::pretty_breaks())+
...
#### Example C: Measuring Building Occupany to Mitigate Risk of Airborne Infections

In this example, an office building has a ventilation system with multiple settings. Increasing the ventilation rate, measured as Air Changes per Hour (ACH), is increasingly expensive, but reduces the risk of infection from airborne pathogens.

Probability of infection $\Pr(inf)$ is predicted using a method developed for calculating the probability of an individual becoming infected by suspended particles in an indoor environment [@DeOliveira2021], as shown below:

$$
\Pr(inf) = 1 - \exp \bigg( \dfrac{-n_{inh}}{C_{r}} \bigg)
$$

Where $n_{inh}$ is iteratively calculated:

$$
n_{inh}[i] = n_{inh}[i-1] + i_{r} \times \Delta{t} \times C[i] \\
C[i] = \dfrac{N * N_{r}}{Vol \times loss} + \dfrac{C[i-1] - N \times Nᵣ}{(Vol \times loss))} \times \exp(-loss \times Δt)
$$

The above equations are detailed in [@Gkantonas2021], but the dependence on the number of individuals using the office, $N$, suggests that knowing this value (by measuring occupancy) will benefit the identification of the expected optimal ventilation rate, each day.

This problem is described in the influence diagram in @fig-vent_id.

```{mermaid}
%%| label: fig-vent_id
%%| fig-cap: "Influence Diagram for Office Ventiliation Setting"

  flowchart LR
  occ((Occupancy)) --> ir((Infection \nRate))
...
Firstly, setting up the infection rate model:

::: {#chunk_load_packages .panel-tabset}

The below functions define the probability of infection and, given this, the probabilities that each number (up to the total number of occupants) are infected.

## Julia (using Distributions)
```{julia}
#| output: false

venting = [1, 3, 5, 10] ./ 3600; κ = 0.39 / 3600; λ = 0.636 / 3600
...
Then defining a prior model for building occupancy, and the costs associated with the various outcomes:

::: {#chunk_load_packages .panel-tabset}

A Poisson distribution is proposed as a prior model for occupancy, with a mean value of 30 people. This was selected since samples from Poisson distributions are positive and integer. An alternative model is a negative binomial distribution. Prior predictive sampling should be used to ensure that the selection represents a suitable estimate for the number of occupants on a given day.

## Julia (using Distributions)
```{julia}
#| output: false

occupancy_model = Poisson(30)
occupancy_samples = occupancy_model |> 
    x -> draw_lhs(x, 1000)
...
Finally, setting up the influence diagram, with these inputs.

::: {#chunk_load_packages .panel-tabset}

The below function defines and solves the influence diagram.
...
The expected value of measuring occupancy is defined as the difference between the expected utility with and without the data. 
...
```{julia}
@printf "VoPI = GBP %.2e" VoPI
```

:::

Although this value is low, it is expected to scale with the number of days that the ventilation rate can be controlled. For instance, this value approaches $£1,000$ after $1$ year. In such a forecast, the prior distribution would have to be a suitable estimate for each day in the upcoming time window. Note that influence diagrams can be extended to introduce time-varying features and opportunities to collect multiple data together [@DiFrancescoa], but these have not been included in the examples in this document.
...
This is an example of how a prior can be identified for a value that is intended to be measured by using other available, relevant information. This method is known as missing data imputation [@Gelman2020].
...
This example is based on a published calculation [@DiFrancesco], which also considers other imperfect features of inspection data, such as reliability. Imperfect data will always be less valuable than perfect data, but perfect data is never available in practice. Increasingly precise and reliable data will have a higher (or at least an equivalent) expected value to a decision-maker and value of information analysis can be used to identify when it is expected to be worth paying more for *better quality* data. 
...
This document will exist online, alongside all necessary data in a Github repository. By making the code and data entirely available, based on principles of Open science @TuringWay and accessibility @Dogucu2022, it is hoped that readers will be able to investigate the contents more thoroughly, understand the methods more deeply, and even propose contributions that can be reviewed and incorporated in future revisions. A final proposition of this document is that other engineering standards consider publishing similarly, whenever and however possible.
...
This work was supported by Wave 1 of The UKRI Strategic Priorities Fund under the EPSRC Grant EP/W006022/1, particularly the ‘Ecosystems of Digital Twins’ theme within that grant, and The Alan Turing Institute. 

The following individuals kindly agreed to review this work and provided helpful comments: Andrew Duncan, Jennifer Ding (both Alan Turing Institute), Lawrence Bull (Cambridge University).
