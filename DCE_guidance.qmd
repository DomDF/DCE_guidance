...
It is possible to sample from complex high-dimensional, correlated distributions using probabilistic programming.
...
General principle: engineering analysis is intended to support decisions [Finad Jordaan quote], but it is necessary to link the analysis to a characterisation of the decision problem, for this to be achieved consistently and coherently.

For example: 
uncertain quantity (informed by an analysis) = applied load on a component. 
Decision = required strength.
Result = A higher strength than believed to be required.
...
This message is made in the below extract, in the context of pipeline engineering:
...
### Expected Value of Imperfect (Imprecise) Data
...
Key concepts:
 ...
 - replicable, auditable, decision making
