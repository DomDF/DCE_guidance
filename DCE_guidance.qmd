...
$$
Where: $\beta_{A} = 0.10$, $\beta_{B} = 1.0$, and $\gamma = 1.50$. The cost of a maintenance activity is equal to $\$2,000$, and is assumed to scale linearly, i.e. $N$ maintenance activities would cost $N \times \$2, 000$.
...
Probability of infection $\Pr(inf)$ is predicted using a method developed for calculating the probability of an individual becoming infected by suspended particles in an indoor environment [@airborne-cam2021, @Oliveira2020], as shown below:
...
The above equations are detailed in [@airborne-cam2021], but the dependence on the number of individuals using the office, $N$, suggests that knowing this value (by measuring occupancy) will benefit the identification of the expected optimal ventilation rate, each day.
