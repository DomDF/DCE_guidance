...
The example calculations in this document, though presented in various levels of detail, are all based on the premise that collecting data reduces statistical uncertainty, and it is possible to check when this is (and is not) expected to be useful.
:::
...
After sampling from the fitted model, it is necessary to extract (and process) the results, so that they can be used to make predictions. Below are some suggested methods of achieving this.
